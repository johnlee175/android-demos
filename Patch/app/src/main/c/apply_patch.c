...
int do_patch_apply(const char *oldfile, const char *newfile, const char *patchfile)
{
    FILE * f, * cpf, * dpf, * epf;
...
jint Java_com_version_increment_patch_Patch_apply(JNIEnv *env, jclass *klass,
                                              jstring oldfile, jstring newfile, jstring patchfile) {
    const char *oldpath = (*env)->GetStringUTFChars(env, oldfile, 0);
    const char *newpath = (*env)->GetStringUTFChars(env, newfile, 0);
    const char *patchpath = (*env)->GetStringUTFChars(env, patchfile, 0);
    return do_patch_apply(oldpath, newpath, patchpath);
}