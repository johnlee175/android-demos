...
public final class ServicesManagerNative extends IServicesManager.Stub {
    ...
    private final ArrayList<IServicesCallback> servicesCallbacks = new ArrayList<>();
    ...
    @Override
    public void setServicesCallback(final IServicesCallback servicesCallback) throws RemoteException {
        servicesCallbacks.add(servicesCallback);
        for (String serviceName : binderMap.keySet()) {
            IBinder binder = binderMap.get(serviceName);
            if (binder instanceof IService) {
                ((IService)binder).onBinderProxyAdded();
            }
        }
        servicesCallback.asBinder().linkToDeath(new DeathRecipient() {
            @Override
            public void binderDied() {
                servicesCallbacks.remove(servicesCallback);
                for (String serviceName : binderMap.keySet()) {
                    IBinder binder = binderMap.get(serviceName);
                    if (binder instanceof IService) {
                        ((IService)binder).onBinderProxyRemoved();
                    }
                }
            }
        }, 0);
    ...
    private void notifyServiceRemoved(String serviceName) {
        for (IServicesCallback callback : servicesCallbacks) {
            try {
                callback.onServiceUnload(serviceName);
